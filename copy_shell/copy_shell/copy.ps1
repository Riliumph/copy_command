...
Write-Host ("[{0}] Get file path from list..." -f [DateTime]::Now )
...
Write-Host ("[{0}] Missing file quantity: {1}" -f [DateTime]::Now, $miss_list.Count)
$miss_list | Set-Content ./miss_list.log -Encoding UTF8

Write-Host ("[{0}] Check destination path..." -f [DateTime]::Now)
...
Write-Host ("[{0}] All is done!!" -f [DateTime]::Now)